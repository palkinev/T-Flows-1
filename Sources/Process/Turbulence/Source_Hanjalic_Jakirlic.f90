...
!-----------------------------------[Locals]-----------------------------------!
  ...
  real    :: eps_huge_var_rename, eps_2_kin_wave
!==============================================================================!
!   Dimensions:                                                                !
...
! but dens > 1 mod. not applied here yet
  ...
  do  c = 1, grid % n_cells
    ...
    ! formula 2.4
    e11 = eps_h_11 / eps % n(c) - TWO_THIRDS
    e22 = eps_h_22 / eps % n(c) - TWO_THIRDS
    e33 = eps_h_33 / eps % n(c) - TWO_THIRDS
    e12 = eps_h_12 / eps % n(c)
    e13 = eps_h_13 / eps % n(c)
    e23 = eps_h_23 / eps % n(c)
    ...
    ! page 165 f
    f_  = min((re_t/150)**1.5, 1.)
    ! page 143 table (not used)
    f_d   = 1./(1. + 0.1*re_t)
    ! page 165 F (not used)
    ff1  = min(0.6, a_2)
    ! page 165 C
    c_  = 2.5*a_*ff1**0.25*f_
    ! page 165 C_1
    c_1  = c_ + sqrt(a_)*e_**2.
    ! page 165 C_2
    c_2  = 0.8*sqrt(a_)
  ...
  do i = 1, 3
    ...
    if(name_phi .eq. 'EPS') then
      ...
      ! page 165 f_eps
      f_eps = 1. - (1. - 1.4/c_2e)*exp(-(re_t/6.)**2.)
      ...
      do c = 1, grid % n_cells
        ...
        A % val(A % dia(c)) = A % val(A % dia(c)) + grid % vol(c) * ( &
          - 1.44 * p_kin(c) / kin % n(c)  & ! page 165, diss. eq., first term
